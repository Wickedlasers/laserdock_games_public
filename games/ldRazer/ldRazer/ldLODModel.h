...
namespace ldRazer
{
    ...
    struct ldLODModel
    {
        struct LODLevel
        {
            ...
            LODLevel(float selector, size_t vtxStart, size_t vtxCount, size_t idxStart, size_t idxCount)
                    : selector(selector), vertexStart(vtxStart), vertexCount(vtxCount), indexStart(idxStart),
                      indexCount(idxCount)
            {
            }
        };
