...
template<typename T>
bool isHull(const std::vector<ldVec2<T>> &vertices)
{
...
bool isHull(const std::array<ldVec2<T>, sides> &vertices)
{
...
struct ldHull
{
    ...
    ldHull(const std::array<vector_t, sides> &vertices) : vertices(vertices)
    {
    ...
    void buildHull(const std::array<vector_t, sides> &vertices)
    {
        if (!isHull(vertices)) return;
...
template<typename T, size_t sides>
ldHull<T, sides> buildHull(const std::array<ldVec2<T>, sides> &vertices)
{
    ldHull<T, sides> hull;
...
template<typename T>
bool intersection(const std::vector<ldVec2<T>> &hull, const ldVec2 <T> &P)
{
...
template<typename T>
std::vector<ldVec2<T>> convexHull(const std::vector<ldVec2<T>> &input)
{
    ...
    std::vector<ldVec2<T>> hull;
    hull.emplace_back(input[edges[0]]);
    hull.emplace_back(input[edges[1]]);

    const ldVec2 <T> *first = &input[edges[0]];
    const ldVec2 <T> *last = &input[edges[1]];
    while (last != first) {
