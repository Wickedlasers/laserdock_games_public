...
#ifndef __LASERDOCK_RAZER_LDAABB_H__
#define __LASERDOCK_RAZER_LDAABB_H__
...
namespace ldRazer
{
    ...
    struct ldAABB
    {
        ...
        ldAABB(const vector_t &centre, T dim) :
                left_bottom(centre - ldVec2<T>(dim, dim)),
                right_top(centre + ldVec2<T>(dim, dim))
        {
        }

        ldAABB(const vector_t &left_bottom, const vector_t &right_top) : left_bottom(left_bottom), right_top(right_top)
        {
        }
...
// Find separating axis, unroll loop
    ...
    template<typename T>
    inline bool intersection(const ldAABB<T> &B, const ldVec2 <T> &P)
    {
