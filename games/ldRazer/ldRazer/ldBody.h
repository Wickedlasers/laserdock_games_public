...
namespace ldRazer
{
    ...
    struct ldBody
    {
        ...
        void setBody(const vec2f &pos, float orn, float mass, const ldBound &modelBound)
        {
            this->pos = pos;
            this->orn = orn;
            this->mass = mass;
            this->invMass = isZero(mass) ? 0.f : 1.f / mass;
        ...
        {
            if (!isZero(invMass)) this->force += force;
        }

        void applyForce(const vec2f &pos, const vec2f &force)
        {
            if (!isZero(invMass)) {
                this->force += force;
                this->torque += dotPerp((pos - this->pos), force);
            }
        }
    };
    ...
    struct ldContact
    {
        ...
        ldContact(const vec2f *conA, const vec2f *conB, uint16_t lenC, const vec2f &N, float t, ldBody *bodyA,
                  ldBody *bodyB)
                : normal(N), t(t), count(0)
        {
            bodies[0] = bodyA;
            bodies[1] = bodyB;
